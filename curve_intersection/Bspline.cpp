...
std::optional<Bspline> Bspline::decompose(double u1, double u2) const
{
	...
	std::cout << std::format("decompose() u1: {}, u2: {}\n", u1, u2);
	...
	{
		std::cout << "decompose(); m = n + p + 1 not satisfied, return\n";
		return {};
	}
	...
	std::cout << std::format("decompose() pre_a: {}, pre_b: {}, m1: {}, m2: {}\n", pre_a, pre_b, m1, m2);
...
void Bspline::findIntersection(Bspline crv, std::vector<Point>& iPoints, int& dNum, bool lineDetection)
{
	...
	{
		std::cout << "findIntersection() negative delta u, return\n";
		return;
	}
	...
	if (deltaU1 > deltaU2)
	{
		std::cout << "switching two curves\n";
		crv.findIntersection(*this, iPoints, dNum, lineDetection);
		return;
	}
	...
	if (deltaU1 < u_epsilon) // prevent only one curve is getting smaller, reduce further iterations
	{
		...
		for (auto& x : crv.convexHull) // or crv.controlPoints
		{
			...
			{
				std::cout << "=== intersection found at control points ===\n";
		...
		{
			std::cout << "=== intersection found between a point and a line segment ===\n";
			iPoints.push_back(pt1);
			std::cout << "returning\n";
			return;
		}
		...
		{
			std::cout << "a point is outside the box containing other's convex hull\n";
			return;
		}
		
		if (deltaU1 < epsilon) // prevent infinite loop(stackoverflow)
		{
			std::cout << "last resort, deltaU1 is too small. stop continuing. return\n";
			return;
		}
		...
		if (convexHull.size() == 2 && crv.convexHull.size() == 2) // two line segments or points
		{
			...
			if (d == 0.0)
			{
				if (coef_c == crv.coef_c)
				{
					...
					{
						std::cout << "findIntersection() *** two line segments are touching or (partially) overlapping ***\n";
					}
					else
					{
						std::cout << "findIntersection() two line segments are on the same line(not overlapping)\n";
					}
				}
				else
				{
					std::cout << "findIntersection(); two line segments are parallel\n";
				}
	...
	std::cout << std::format("min, max of clipping lines of curve A: {}, {}\n", minDist, maxDist);

	if (maxDist == minDist)  // (std::abs(maxDist - minDist) < epsilon)
	{
		std::cout << std::format("curve A is a line segment\n");
	}
	...
	std::cout << std::format("minimum and maximum of distance curve: {}, {}\n", min, max);

	if (min > maxDist || max < minDist) // outside the clipping lines: no intersection
	{
		std::cout << "curve B is outside the clipping lines\n";
		return;
	}
	...
	if ((minDist < pt.y && pt.y < maxDist) || (minDist < pt2.y && pt2.y < maxDist))  // on boundary decompose curve B. cf) if ((minDist <= pt.y && pt.y <= maxDist) || (minDist <= pt2.y && pt2.y <= maxDist)) on boundary decompose curve A
	{
		...
		{
			++dNum;
			bs1->findIntersection(crv, iPoints, dNum, lineDetection);
		}
		auto bs2{ decompose(knotVector[0] + deltaU1 / 2.0, knotVector[cp_n + 1]) };
		if (bs2)
		{
			++dNum;
			bs2->findIntersection(crv, iPoints, dNum, lineDetection);
		}
		return;
	} //end if
	...
	auto findUs = [&](double distance)
	{
		...
		{
			std::cout << "intersection between clipping line and one edge of convex hull of the distance curve: \n";
			if (pt.y == pt2.y)
			{
	...
	if (uValues.size() < 2)
	{
		std::cout << "the number of clipped u values < 2, returning\n";
		return;
	}
	...
	{
		std::cout << "u value is NaN, returning\n";
		return;
	}

	if (std::isinf(u_min) || std::isinf(u_max))
	{
		std::cout << "u vaue is Inf, returning\n";
		return;
	}

	if (u_max == u_min)
	{
		std::cout << "The interval of the u on the clipped curve is zero: a point\n";
	}

	std::cout << std::format("u_min: {}, u_max: {}\n", u_min, u_max);
	...
	if ((u_max - u_min) > deltaU2 * 0.8)
	{
		...
		{
			++dNum;
			bs1->findIntersection(*this, iPoints, dNum, lineDetection);
		}
		auto bs2{ crv.decompose(crv.knotVector[0] + deltaU2 / 2.0, crv.knotVector[crv.cp_n + 1]) };
		if (bs2) {
			++dNum;
			bs2->findIntersection(*this, iPoints, dNum, lineDetection);
		}
		return;
	}
	else
	{
		...
		{
			++dNum;
			decomposedC2->findIntersection(*this, iPoints, dNum, lineDetection);
		}
		return;
	}
	...
	for (size_t i{}; i < bezierLists[0].size(); ++i) {
		for (size_t j{}; j < bezierLists[1].size(); ++j)
		{
			if (bezierLists[0][i].has_value() && bezierLists[1][j].has_value())
				bezierLists[0][i]->findIntersection(*bezierLists[1][j], iPoints, dNum, lineDetection);
		}
	}
